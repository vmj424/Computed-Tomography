...
yc = yc+ccc;
% test yc1 and yc2 before continuing and yt1 thereafter
...
subplot(2,5,4), imshow((reshape(Mean,[N,N]))), xlabel('Arithmetic Mean'),set(gca,'FontSize',14)
% title(['Accuracy Error = ' , num2str(A3)])
% ylabel(['Measurement Error = ', num2str(M3)])
subplot(2,5,5), imshow((reshape(Weighted_average,[N,N]))), xlabel('Weighted Average'),set(gca,'FontSize',14)
% title(['Accuracy Error = ' , num2str(A4)])
% ylabel(['Measurement Error = ', num2str(M4)])
subplot(2,5,6), imshow((reshape(Product,[N,N]))), xlabel('Geometric Mean'),set(gca,'FontSize',14)
% title(['Accuracy Error = ' , num2str(A5)])
% ylabel(['Measurement Error = ', num2str(M5)])
subplot(2,5,7), imshow((reshape(Min,[N,N]))), xlabel('Minimum'),set(gca,'FontSize',14)
% title(['Accuracy Error = ' , num2str(A6)])
% ylabel(['Measurement Error = ', num2str(M6)])
subplot(2,5,8), imshow((reshape(Max,[N,N]))), xlabel('Maximum'),set(gca,'FontSize',14)
% title(['Accuracy Error = ' , num2str(A8)])
...
% different fusion methods 
Mean = mean(B,2);
Weighted_average = 0.7*(B(:,1)) + 0.3*(B(:,2));
Product = real(sqrt(dot(B(:,1),B(:,2),2)));
Min = min(B,[],2);
Median = median(B,2); 
Max = max(B,[],2);
