...
%
figure(01)
subplot(1,2,1),imshow(reshape(xf,N,N)), xlabel('Actual'),set(gca,'FontSize',16)
subplot(1,2,2),imshow(reshape(Xc_t1,n,n)), xlabel('Coarse'),set(gca,'FontSize',16)
colormap gray

figure(02)
subplot(1,2,1), histogram(xf,100), title('Actual'), xlabel('Grayscale value'), ylabel('Number of pixels'),set(gca,'FontSize',16)
subplot(1,2,2), histogram(Xc_t1,100),title('Coarse'), xlabel('Grayscale value'), ylabel('Number of pixels'),set(gca,'FontSize',16)
%% I = x;
p= N;
q =N;  
...
% fuzzy Fusion 
fis = readfis('Triangle_FUSION8'); % Triangle_FUSION9 when ideal case 
...
fis2 = readfis('SingleFIS8'); % SingleFIS9 when ideal case
                              % SingleFIS8 when noisy case
input =  [x1(:)];
