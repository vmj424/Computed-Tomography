...
[Ac,yc,xc,theta,p,d] = paralleltomo(n,theta,p,d_c);
%% load stored A_collapsed matrix 
load('A_collapsed.mat')
%%
...
xxx = norm((Xc_t1)-(Xc_t1_2))/norm(Xc_t1);
...
figure(14)
subplot(1,2,2),imshow(reshape(U,N,N)), xlabel('TV-regularization');
title(['RRMSE (%) = ', num2str(RRMSE)])%, xlabel('10643 elements in A with 29 projections')
subplot(1,2,1),imshow(reshape(xf,N,N)),xlabel('Origional');
colormap gray
