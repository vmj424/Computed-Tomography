% First selct appropriate phantom from phantom gallery and change in line 100
clear
clc
close all
...
% fuzzy Fusion 
fis = readfis('Threephase_fusion_TV_noise'); % threephase_fusion.fis for noisy case when DoD is 0.84
                                    % Threephase_fusion_ideal.fis for ideal
                                    % cae both 0.84 and 0.42
                                    % Threephase_fusion_TV_noise is for
                                    % 0.42 DoD when noisy case
...
fis2 = readfis('threephase_TV_noise'); % Threephase.fis for noisy case when DoD is 0.84 
                              % Threephase_ideal.fis for ideal case in DoD 0.84
                              % and 0.42
                              % threephase_TV_noise.fis for noisy case when
                              % DoD is 0.42
...
% su = sort(fuzzy2(:));
% figure(999)
% subplot(1,3,1), stairs(xu), xlabel('Original')
% subplot(1,3,2), stairs(mu), xlabel('TV-regularization')
% subplot(1,3,3), stairs(su), xlabel('Image refinement')