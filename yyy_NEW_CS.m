...
%% load stored A_collapsed matrix for DoD 0.84
...
cvx_begin
variable s1(N^2)
minimize norm(Theta*s1 -yf) + lambda*norm(s1,1)
cvx_end
...
figure(11)
subplot(1,2,2),imagesc(reshape(U(:),N,N)), xlabel('CS');
title(['RRMSE (%) = ', num2str(RRMSE)])%, xlabel('')
