...
p = 185;
Maxtheta = 179;
    theta = 0:4.8:Maxtheta; % TWO DoD 4.8 and 2.4
    ...
    d_c = round(sqrt(2.1)*n); % first to last beam distance in coarse 
%    d_f = d_c*k;
%     d_c = round(sqrt(2)*n); % first to last beam distance in coarse 
     d_f = 182;  % first to last beam distance in fine 
...
% [Af,yf,xf,thetaf,pf,df] = paralleltomo(N,theta,p); 

% Construction of coarse matrix 
[Ac,yc,xc,theta,pc,d] = paralleltomo(n,theta,p,d_c);

% [Ac,yc,xc,theta,pc,d] = paralleltomo(n,theta,p);
...
% Enlarge coarse to obtain adjusted fine 
x = kron(reshape(Xc_t1,n,n),ones(k)); % Fine image from actual coarse phantom
x = x(:);
y_coarse = Af*x;
...
fis2 = readfis('SingleFIS4'); % use 'SingleFIS3' when fusing 16X16 with TV and 'SingleFIS4'  
                               % when fusing 32X32
input =  [x1(:)];
